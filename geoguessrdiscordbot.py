...
async def geo(ctx, arg1, arg2):
    ...
    if user_map in maps and rule in options:
        ...
        generatelink_message.set_footer(icon_url= author.avatar_url, text = f"Game requested by {author.display_name}({author.username}) ")
        ...
        game_message.set_footer(icon_url= author.avatar_url, text = f"Game generated by {author.display_name}({author.username})")
...
async def on_command_error(ctx,error):
    ...
    if isinstance(error, CommandOnCooldown):
        ...
        errorEmbed.set_footer(icon_url = author.avatar_url, text = f"Just relax, {author.display_name}({author.username}) :)")
        await ctx.send(embed = errorEmbed)
