...
async def geo(ctx, arg1, arg2):
    ...
    server_file = f"{ctx.guild.id}.txt"
    user_map = arg1.lower()
    rule = arg2.lower()
...
async def last_error(ctx,error):
    ...
    if isinstance(error, commands.CommandInvokeError):
        errorEmbed.add_field(name = "Uh oh!", value = "Looks like the configuration of the last game has expired. Please generate a new GeoGuessr link to store a game configuration.", inline = False )
        await ctx.send(embed = errorEmbed)
        ctx.command.reset_cooldown(ctx)
    else: 
        print(error)
